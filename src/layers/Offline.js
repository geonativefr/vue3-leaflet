...
async function checkMap(map) {
  ...
  const urls = ZOOM_LEVELS.map((zoomLevel) => {
    const area = bounds(CRS.EPSG3857.latLngToPoint(map.NE, zoomLevel), CRS.EPSG3857.latLngToPoint(map.SW, zoomLevel));
    return getTileUrls(
  ...
  const savedUrls = new Set(
    await readAllKeysIndex(state.db, TABLES.TILES.name, TABLES.TILES.indexes.MAP, IDBKeyRange.only(map.normalizedName))
  );
